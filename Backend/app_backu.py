...
class UserAchievement(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    achievement_id = db.Column(db.Integer, db.ForeignKey('shop_items.id'), nullable=False)
    unlocked_at = db.Column(db.DateTime, default=datetime.utcnow)

    user = db.relationship('User', backref='user_achievements')
    achievement = db.relationship('ShopItem', backref='user_achievements')
...
def unlock_achievement():
    ...
    exists = UserAchievement.query.filter_by(user_id=current_user_id, achievement_id=achievement_id).first()
    if exists:
        return jsonify({"msg": "Achievement already unlocked"}), 400

    # Optionally: Check if achievement exists and is of category 'achievement'
    achievement = ShopItem.query.get(achievement_id)
    if not achievement or achievement.category != 'achievement':
        return jsonify({"msg": "Invalid achievement"}), 404

    ua = UserAchievement(user_id=current_user_id, achievement_id=achievement_id)
    db.session.add(ua)
    db.session.commit()
    return jsonify({"msg": "Achievement unlocked!"}), 201
...
def get_all_achievements():
    achievements = ShopItem.query.filter_by(category='achievement').all()
    result = [{
        "id": a.id,
        "name": a.name,
        "icon": a.icon,
...
def get_daily_claim_status():
    ...
    if user.last_daily_reward_claim:
        ...
        next_claim_time = last_claim + timedelta(days=1)  # For testing; change to timedelta(days=1) for production
    else:
        next_claim_time = now
...
def update_fitte_points():
    ...
    # Check if this is a daily reward claim
    if is_daily_reward:
        ...
        if user.last_daily_reward_claim:
            ...
            if last_claim.tzinfo is None:
                last_claim = last_claim.replace(tzinfo=timezone.utc)
            if (now - last_claim) < timedelta(seconds=10):
                return jsonify({"msg": "You can only claim the daily reward once per day"}), 400
                
        # ✅ Update the timestamp for daily claim
        user.last_daily_reward_claim = now
        socketio.emit('daily_reward_claimed', {'user_id': user_id}, namespace='/socket')
    ...
    if user.last_daily_reward_claim:
        next_claim_time = (user.last_daily_reward_claim + timedelta(seconds=10)).isoformat()
